...
#endif        
		sum=0; sum2=0;     
		#pragma omp parallel default(none) shared(J, sum, sum2) private(i, j, tmp)
		{
		#pragma omp for private(j, tmp) reduction(+: sum, sum2) schedule(auto)
		for (i=r1; i<=r2; i++) {
            #pragma omp simd reduction(+: sum, sum2) private(tmp)
            for (j=c1; j<=c2; j++) {
                tmp   = J[i * cols + j];
                sum  += tmp ;
                sum2 += tmp*tmp;
